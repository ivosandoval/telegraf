...
func NewRotatingWriter(root, prefix string) (*Writer, error) {
	l := &Writer{root: root, prefix: prefix, max: maxTimeDefault, keep: keepDefault}
...
// SetMax sets the maximum age for a file.
func (r *Writer) SetMax(age time.Duration) {
	r.max = age
}
...
func (r *Writer) Write(p []byte) (n int, err error) {
	...
	if time.Now().After(r.currentCreationTime.Add(r.max)) {
...
func (r *Writer) openCurrent() error {
	...
	r.currentCreationTime = time.Now()
	if err != nil {
		return err
	}
	return nil
